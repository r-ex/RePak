...
void AutoAddMaterialAssetTextures(CPakFile* pak, std::vector<PakAsset_t>* assetEntries, rapidjson::Value& mapEntry)
{
    ...
    if (mapEntry.HasMember("textures") && mapEntry["textures"].IsArray())
    {
        for (auto& it : mapEntry["textures"].GetArray())
        {
            ...
            Assets::AddTextureAsset(pak, assetEntries, it.GetString(), mapEntry.HasMember("disableStreaming") && mapEntry["disableStreaming"].GetBool());
        }
    }
}
...
void SetUpMaterialAssetFromJson(MaterialAsset_t* matl, rapidjson::Value& mapEntry)
{
    ...
    if (mapEntry.HasMember("width")) // Set material width.
        matl->width = mapEntry["width"].GetInt();

    if (mapEntry.HasMember("height")) // Set material width.
        matl->height = mapEntry["height"].GetInt();
    ...
    if (mapEntry.HasMember("surface"))
        matl->surface = mapEntry["surface"].GetStdString();

    // used for blend materials and the like
    if (mapEntry.HasMember("surface2"))
        matl->surface2 = mapEntry["surface2"].GetStdString();
    ...
    // texan
    if (mapEntry.HasMember("textureAnimation") && mapEntry["textureAnimation"].IsString())
