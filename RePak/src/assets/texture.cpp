	...
	if (mapEntry.HasMember("textures") && mapEntry["textures"].IsArray())
	{
		...
		for (auto& entry : mapEntry["textures"].GetArray())
		{
			if (entry.IsString() && pak->GetAssetByGuid(RTech::StringToGuid(entry.GetString())) == nullptr)
				TextureList.push_back(entry.GetStdString());
		}


		AddTextureListAsset_v8(pak, assetEntries, g_sAssetsDir, TextureList);
	}
}
...
void Assets::AddTextureAsset_v8(CPakFile* pak, std::vector<RPakAssetEntry>* assetEntries, const char* assetPath, rapidjson::Value& mapEntry)
{
	...
	ParseImage(input, hdr, nLargestMipSize, nStreamedMipSize, nDDSHeaderSize, bStreamable, assetPath, true);
...
{
	bool bSaveDebugName = pak->IsFlagSet(PF_KEEP_DEV);

	uint64_t DebugNameSize = 0;
	uint64_t DataSize = 0;
	for (auto& Texture : TextureList)
	{
		std::string filePath = sAssetsDir + Texture + ".dds";

		if (!FILE_EXISTS(filePath))
			Error("Failed to find texture source file %s. Exiting...\n", filePath.c_str());

		DebugNameSize += Texture.length() + 1;
		...
		// parse input image file
		ParseImage(input, hdr, nLargestMipSize, nStreamedMipSize, nDDSHeaderSize, bStreamable, Texture.c_str());

		DataSize += hdr->dataSize - nStreamedMipSize;

		input.close();
	}

	// create segments
	uint64_t HeaderSize = sizeof(TextureHeader) * TextureList.size();
	char* pHeaderBuf = new char[HeaderSize];
	_vseginfo_t subhdrinfo = pak->CreateNewSegment(HeaderSize, SF_HEAD, 8);

	char* pNameBuf = new char[DebugNameSize];
	_vseginfo_t nameseginfo{};
	if (bSaveDebugName)
		nameseginfo = pak->CreateNewSegment(DebugNameSize, SF_DEV | SF_CPU, 1);
	else delete[] pNameBuf;

	_vseginfo_t dataseginfo = pak->CreateNewSegment(DataSize, SF_CPU | SF_TEMP, 16);
	...
	for (auto& Texture : TextureList)
	{
		...
		hdr->guid = RTech::StringToGuid((Texture + ".rpak").c_str());

		if (bSaveDebugName)
		{
			snprintf(pNameBuf + nameoffset, Texture.length() + 1, "%s", Texture.c_str());

			hdr->pName = { nameseginfo.index , nameoffset };
			nameoffset += Texture.length() + 1;
		}
		...
		// parse input image file
		ParseImage(input, hdr, nLargestMipSize, nStreamedMipSize, nDDSHeaderSize, bStreamable, Texture.c_str(), true);

		int currentDDSOffset = 0;
		int remainingDDSData = hdr->dataSize;
		int remainingStreamedData = nStreamedMipSize;

		char* pStreamBuf = new char[nStreamedMipSize];
		...
		for (int ml = 0; ml < (hdr->mipLevels + hdr->streamedMipLevels); ml++)
		{
			...
			{
				remainingStreamedData -= nCurrentMipSize;
				input.getReader()->read(pStreamBuf + remainingStreamedData, mipSizeDDS);
			}
			else input.getReader()->read(pDataBuf + dataoffset + remainingDDSData, mipSizeDDS);
		}
		...
		if (bStreamable)
		{
			...
			starpakOffset = pak->AddStarpakDataEntry({ 0, nStreamedMipSize, (uint8_t*)pStreamBuf }).m_nOffset;
		}

		memcpy(pHeaderBuf + headeroffset, &*(TextureHeader*)hdr, sizeof(TextureHeader));

		RPakAssetEntry asset;
		asset.InitAsset(hdr->guid, subhdrinfo.index, headeroffset, sizeof(TextureHeader), dataseginfo.index, dataoffset, starpakOffset, -1, (std::uint32_t)AssetType::TXTR);
		asset.version = TXTR_VERSION;
		...
		headeroffset += sizeof(TextureHeader);
		dataoffset += hdr->dataSize - nStreamedMipSize;
	}

	pak->AddRawDataBlock({ subhdrinfo.index, subhdrinfo.size, (uint8_t*)pHeaderBuf });

	if (bSaveDebugName)
		pak->AddRawDataBlock({ nameseginfo.index, nameseginfo.size, (uint8_t*)pNameBuf });

	pak->AddRawDataBlock({ dataseginfo.index, dataseginfo.size, (uint8_t*)pDataBuf });
}

void ParseImage(BinaryIO& input, TextureHeader*& hdr, uint32_t& nLargestMipSize, uint32_t& nStreamedMipSize, uint32_t& nDDSHeaderSize, bool& bStreamable, const char* assetPath, bool printlogs)
{
	int magic;
	input.read(magic);
